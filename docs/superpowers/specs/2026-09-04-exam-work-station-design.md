...
**v1 数据源**：粉笔公开真题（fenbi.com / 粉笔 App 公开题库导出的 Excel / JSON / PDF 解析后的结构化数据）
...
- v1 只需要 1 个粉笔国考行测导入脚本（5 模块各 50-200 题作为种子）
- v2 扩充：粉笔省考/事业编/选调/公安
- 导入脚本不入前端 bundle，独立 CLI
- 脚本只读用户下载/导出的本地文件，不调用粉笔任何 API（避免账号/合规风险）
